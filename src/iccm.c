...
void iccm_send(char *str)
{
    ...
    for(i = 0; *(str+i) != '\0' && i < ICCM_MAX_DATA_LENGTH; i++)
    {
        ...
        for(j = 0; j < (ICCM_START_BIT+ICCM_DATA_SIZE+ICCM_NUM_STOP_BITS); j++)
        {
            ...
            serial_send(FILE_ID, NOTIFY, str+i);
...
uint8_t iccm_receive_char(char c)
{
    ...
    {
        serial_send(FILE_ID, WARNING, "ICCM RX BUFFER OVERFLOW");
    } 
    else 
    {
        *rx_buffer_ptr = c;
        rx_buffer_ptr++;
        result = 1;
        serial_send(FILE_ID, NOTIFY, rx_buffer_ptr-1);
    }
    return result;
}
...
void iccm_receive()
{
    if(rx_complete_flag)
    {
        ...
        serial_send(FILE_ID, NOTIFY, rx_buffer_ptr);
        rx_buffer_ptr = NULL;
...
void iccm_set_rx_complete_flag(uint8_t value)
{
    ...
    serial_send(FILE_ID, NOTIFY, "rx flag changed");
}
